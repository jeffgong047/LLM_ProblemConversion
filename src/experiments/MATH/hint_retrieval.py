...
@guidance
def combinatorial_hint_search(llm,problem):
    program<....



@guidance
def energy_function(llm, problem, hints):
    '''
    The evaluation result from LLM
    '''
    llm = llm + f'''Could you evaluate the goodness of the hint based on the reasoning process and solution you got using 
    that hint? Please give an evaluation from 1-10. Here is the problem: {problem}, here are the hints: {hints} '''+ gen(name='energy',max_tokens=50)
    return llm['energy']
@guidance
def neighbor_solution(llm,hints):
    '''
    '''
    llm  = llm+f'''Please propose a few hints that is close to current hints:{hints},
     you can for instance replace some of the hints in the current combination''',+gen(name='next_hints',max_tokens = 50)
    return llm['next_hints']

def simulated_annealing(initial_solution, current_cost, temperature, cooling_rate, stop_temperature):
    current_solution = initial_solution

    while temperature > stop_temperature:
        new_solution = generate_new_solution(current_solution)
        new_cost = energy_function(new_solution)
...
def judger(llm,*args, **kwargs):
    ...
    Is the final_answer correct, given the ground truth answer?''' + select(['Correct', 'Wrong', 'Unknown'],name ='correctness')+ f'''Please provide the reason for your judgement and how confident you are about your judgement on the scale from 1-10.'''+gen(name='notes',max_tokens = 50)
...
Here is the prblem, Q:{kwargs['question']} ,H:Please provide few but less than 5 math theorems that are most representative as hints first before generating the solution, please dont provide explanation:''' +gen(max_tokens=50, name='hints')+f'''S: Please provide the solution based on the hints:''' + gen(max_tokens=1000, name = 'final_solution')
    return llm
def main():
